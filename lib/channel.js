...
class Client {
  ...
  pipe(name, stream) {
    ...
    const packet = { stream: callId, name };
    this.connection.send(JSON.stringify(packet));

    stream.on('data', chunk => {
      const packet = { stream: callId, size: chunk.length };
      this.connection.send(JSON.stringify(packet));
      this.connection.send(chunk);
    });

    stream.on('end', () => {
      const packet = { stream: callId, status: 'end' };
      this.connection.send(JSON.stringify(packet));
    });

    stream.on('error', () => {
      const packet = { stream: callId, status: 'terminate' };
      this.connection.send(JSON.stringify(packet));
    });
