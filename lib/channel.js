...
class Client {
  ...
  getStream(streamId) {
    const channel = channels.get(this);
    if (!channel.connection || !channel.stream) {
      throw new Error(`Can't receive stream from http1 transport`);
    }
  ...
  createStream(name, size) {
    const channel = channels.get(this);
    if (!channel.connection || !channel.stream) {
      throw new Error(`Can't send metacom streams to http1 transport`);
    }
    ...
    const transport = channel.connection || channel.stream;
