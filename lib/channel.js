...
class Channel {
  ...
  message(data) {
    ...
    if (callType === 'call') return this.handleRpcPacket(packet);
    else if (callType === 'stream') return this.handleStreamPacket(packet);
  ...
  async rpc(callId, interfaceName, methodName, args) {
    ...
    try {
      ...
      const timeout = error.message === 'Timeout reached';
      const code = timeout ? 408 : 500;
      this.error(code, { callId, error, pass: timeout });
      return;
    } finally {
      proc.leave();
    }
