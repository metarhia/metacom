...
class Channel {
  ...
  async rpc(callId, interfaceName, methodName, args) {
    ...
    try {
      ...
      const isTimeout = error.message === 'Timeout reached';
      const httpCode = error.httpCode || (isTimeout ? 408 : 500);
    ...
    if (
      ...
      result.constructor.name === 'Error'
    ) {
      const { code, httpCode } = result;
      this.error(code, { callId, error: result, httpCode });
      return;
    }
  ...
  error(code = 500, { callId, error = null, httpCode = 500 } = {}) {
    ...
    const shouldPass = httpCode < 500;
    const message =
      shouldPass && error ? error.message : status || 'Unknown error';
