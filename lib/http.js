...
class HttpChannel extends Channel {
  ...
  startSession() {
    const token = this.server.application.auth.generateToken();
    const host = metautil.parseHost(this.req.headers.host);
    const cookie = `${TOKEN}=${token}; ${COOKIE_HOST}=${host}`;
    const session = this.server.application.auth.startSession();
  ...
  deleteSession() {
    ...
    this.server.application.auth.deleteSession(token);
  }
}
