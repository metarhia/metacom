...
const Client = function(
  ...
  this.transportType = options.transport; // ws, wss, tcp, tls
  this.parcelId = 0;
...
Client.prototype.connect = function(callback) {
  this.transport.connect(this.port, this.host, () => {
    ...
    const onHandshake = ({ status, token }) => {
      if (status === HANDSHAKE_STATUS.accept) {
        this.active = true;
        this.token = token;
        if (callback) callback();
        this.emit('connect');
      } else if (status === HANDSHAKE_STATUS.reject) {
...
Client.prototype.restore = function(callback) {
  ...
  this.transport.connect(this.port, this.host, () => {
    const handshake = {
      status: HANDSHAKE_STATUS.restore,
      reserved: 0,
      token: this.token
    };
