...
const messageHandlers = {
  ...
  event: async (client, { unit, name, meta, data }) => {
    const metacomUnit = client.api[unit];
    if (metacomUnit) metacomUnit.emit(name, { ...data, meta });
  },
...
class Metacom extends EventEmitter {
  ...
  async load(...units) {
    ...
    for (const unit of units) {
      ...
      methods.on('*', (name, data) => {
        const packet = protocol.serialize('event', { unit, name, data });
