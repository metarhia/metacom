...
const { readStructType, readPayloadLength } = require('./parser-utils');
...
const Connection = function() {
  ...
  this.bytesToRead = parser.HANDSHAKE_SIZE;

  this.transport.on('data', data => {
    if (this.bytesToRead === 0) {
      const structType = readStructType(data);
      this.bytesToRead =
        structType === parser.STRUCT_PARCEL_HEADER
          ? parser.PARCEL_SIZE
          : parser.CHUNK_HEADER_SIZE;
    }
...
Connection.prototype._onRawData = function(data) {
  if (data.length < this.bytesToRead) {
    ...
    this.position += data.copy(this.buffer, this.position, 0, this.bytesToRead);
    const structType = readStructType(this.buffer);

    data = data.slice(this.bytesToRead);

    if (
      structType === parser.STRUCT_CHUNK_HEADER &&
      this.buffer.length === parser.CHUNK_HEADER_SIZE
    ) {
      const length = readPayloadLength(this.buffer);
...
Connection.prototype.sendHandshake = function(handshake) {
  const buffer = writer.writeHandshake(handshake);
  this.send(buffer);
};

Connection.prototype.sendParcel = function(parcel, callback) {
  const buffer = writer.writeParcelHeader(parcel, this.encoding);
  this.send(buffer, callback);
};

Connection.prototype.sendChunk = function(chunk, callback) {
  const buffer = writer.writeChunk(chunk, this.encoding);
...
Connection.prototype._onHandshake = function(buffer) {
  ...
  this.on('structure', buffer => {
    ...
    if (structType === parser.STRUCT_PARCEL_HEADER) {
      const struct = parser.parseParcelHeader(buffer);
      this._onParcel(struct);
    } else if (structType === parser.STRUCT_CHUNK_HEADER) {
      const struct = parser.parseChunk(buffer);
      this._onChunk(struct);
    }
  });
};
