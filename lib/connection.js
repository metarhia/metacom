...
const onChunkSymbol = Symbol('onChunk');
const onParcelSymbol = Symbol('onParcel');
const onRawDataSymbol = Symbol('onRawData');
const onHandshakeSymbol = Symbol('onHandshake');
const emitParcelTypeSymbol = Symbol('emitParcelType');
...
class Connection extends EventEmitter {
  constructor() {
    ...
    this.transport.on('data', (data) => {
      ...
      this[onRawDataSymbol](data);
    });

    // Process handshake as a first structure
    this.once('structure', this[onHandshakeSymbol]);
  }

  [onRawDataSymbol](data) {
    if (data.length < this.bytesToRead) {
      ...
      if (data.length > 0) this[onRawDataSymbol](data);
    }
  }
  ...
  [onHandshakeSymbol](buffer) {
    ...
    this.on('structure', (buffer, length) => {
      ...
      if (struct.structType === parser.STRUCT_PARCEL) {
        this[onParcelSymbol](struct);
      } else if (struct.structType === parser.STRUCT_CHUNK) {
        this[onChunkSymbol](struct);
      }
    });
  }

  [onParcelSymbol](parcel) {
    if (parcel.parcelType === parser.PARCEL_STREAM) {
      this[emitParcelTypeSymbol](parcel);
    }
    ...
    if (currentLength === parcel.length) this[emitParcelTypeSymbol](parcel);
  }
