...
const { Emitter, receiveBody, parseCookies, parseHost } = require('metautil');
...
class ServerTransport extends Emitter {
  constructor(server, req) {
    ...
    this.headers = server.headers || HEADERS;
  }
...
class ServerHttpTransport extends ServerTransport {
  static async handleRequest(server, req, res) {
    const transport = new ServerHttpTransport(server, req, res);
    const { application, balancer } = server;
    ...
    if (balancer) server.balancing(transport);
    if (res.writableEnded) return;

    const client = server.createClient(transport);
    const data = await receiveBody(req).catch(() => null);

    if (req.url === '/api') {
      if (req.method !== 'POST') transport.error(403);
      else server.message(client, data);
      return;
    }
    server.request(client, transport, data);
  }
  ...
  write(data, httpCode = 200, ext = 'json', options = {}) {
    ...
    const headers = { ...this.headers, 'Content-Type': mimeType };
    if (httpCode === 206) {
...
class ServerWsTransport extends ServerTransport {
  static handleConnection(server, req, connection) {
    const transport = new ServerWsTransport(server, req, connection);
    const client = server.createClient(transport);

    connection.on('message', (data, isBinary) => {
      if (isBinary) server.binary(client, new Uint8Array(data));
      else server.message(client, data);
    });
  }
...
class ServerEventTransport extends ServerTransport {
  static handleConnection(server, req, port) {
    const transport = new ServerEventTransport(server, req, port);
    const client = server.createClient(transport);
    port.on('message', (data) => {
      if (typeof data === 'string' || Buffer.isBuffer(data)) {
        server.message(client, data);
      } else if (data instanceof Uint8Array) {
        server.binary(client, data);
      }
    });
  }

  constructor(server, req, port) {
    super(server, req);
