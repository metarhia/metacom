...
class Metacom extends Emitter {
  ...
  constructor(url, options = {}, TransportClass) {
    super();
    ...
    this.url = url;
    const Transport = TransportClass || Metacom.transport.ws;
    this.transport = new Transport(url, this);
  ...
  static create(url, options = {}) {
    ...
    const TransportClass = url.startsWith('ws') ? transport.ws : transport.http;
    return new Metacom(url, options, TransportClass);
  }
...
class WsClientTransport extends ClientTransport {
  ...
  async open() {
    ...
    this.active = true;
    this.socket = socket;
    Metacom.connections.add(metacom);

    const onError = (error) => metacom.emit('error', error);
    socket.addEventListener('message', ({ data }) => {
      const isString = typeof data === 'string';
      const promise = isString
        ? metacom.handlePacket(data)
        : metacom.binary(data);
    ...
    socket.addEventListener('close', () => {
      ...
      }, metacom.reconnectTimeout);
    });

    socket.addEventListener('error', (error) => {
      metacom.emit('error', error);
      socket.close();
    });
  ...
  close() {
    this.active = false;
    Metacom.connections.delete(this.metacom);
...
class HttpClientTransport extends ClientTransport {
  ...
  send(data) {
    const { metacom } = this;
    metacom.lastActivity = Date.now();
    const body = JSON.stringify(data);
    const headers = { 'Content-Type': 'application/json' };
    fetch(this.url, { method: 'POST', headers, body })
      .then((res) => res.text())
      .then((packet) => metacom.handlePacket(packet))
      .catch((error) => metacom.emit('error', error));
  }
}
...
class EventClientTransport extends ClientTransport {
  ...
  handleMessage(event) {
    ...
    const promise = isString
      ? metacom.handlePacket(data)
      : metacom.binary(data);
    promise.catch((error) => metacom.emit('error', error));
  }
...
class MetacomProxy extends Emitter {
  ...
  async open(options = {}) {
    ...
    this.connection = new Metacom(url, opts);
