...
const handleHook = async (client, proc, packet, verb, headers) => {
  const { id, method, args } = packet;
  if (!proc) return void client.error(404, { id });
  try {
    const par = { verb, method, args, headers };
    const res = await protectedCall(client, proc, id, par);
    if (!res.entered) return;
    if (isError(res.result)) {
      const { code } = res.result;
      return void client.error(code, { id, error: res.result });
    }
    client.send(res.result, { method });
  } catch (error) {
...
class Server extends Emitter {
  ...
  #request(client, transport, data) {
    ...
    if (hook) handleHook(client, hook, packet, verb, headers);
    else handleRpc(client, packet, this.#context);
  }
