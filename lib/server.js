...
class Server {
  ...
  bind() {
    ...
    const { host, port, kind, protocol, timeouts, nagle = true, http2tls = false } = options;
    let proto = protocol === 'http' || kind === 'balancer' ? http : https;
    if (protocol == 'http2') proto = http2;
    const listener = this.listener.bind(this);
    this.server = http2tls ? 
      proto.createSecureServer({ ...application.cert }, listener) : 
      proto.createServer({ ...application.cert }, listener);
    if (!nagle) {
  ...
  listener(req, res) {
    ...
    const channel = transport[protocol].createChannel(this.application, req, res);
  ...
  request(channel) {
    ...
    const body = req.stream ? metautil.receiveBody(req) : metautil.receiveBody(req.stream);
    if (req.url === '/api') {
      body.then((data) => {
