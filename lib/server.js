...
const inspectionMethods = {
  ...
  listInterfaces: interfaces => {
    ...
    for (const interfaceName in interfaces) {
      ...
      schema[interfaceName].methods = [];

      const { description } = interfaces[interfaceName];

      if (description)  schema[interfaceName].description = description;

      const methodNames = Object.keys(interfaces[interfaceName]);
      methodNames
        .filter(name => name !== 'description')
        .forEach(name => {
          const fn = interfaces[interfaceName][name];
          const description = { name, length: fn.length };
          schema[interfaceName].methods.push(description);
        });
    }
  ...
  //            ...
  //          }
  //          where `methodNames` is an array of names of
  //          interface methods
  //
  selectApplication: (interfaces, interfaceNames) => {
    const existingInterfaces = Object.keys(interfaces);
    const allInterfaces = interfaceNames[0] === '*';

    if (allInterfaces)  interfaceNames = existingInterfaces;

    const definition = {};

    interfaceNames.forEach(interfaceName => {
      if (!existingInterfaces.includes(interfaceName)) {
        const error = `Interface does not exists: ${interfaceName}`;
        return [error];
      }

      const methods = interfaces[interfaceName];
      definition[interfaceName] = Object.keys(methods);
    });
...
// Process call which was made to inspection interface
// - call - Metacom call object
// - apps - Metacom.Server applications index
//
// Returns: tuple [error, result]
//
const inspection = (call, apps) => {
  const { version, methodName, args } = call;
  const fn = inspectionMethods[methodName];

  if (methodName === 'listApplications') return [null, fn(apps)];

  const app = apps.get(call.application);
  if (!app) return [APP_NOT_FOUND];

  const interfaces = app.get(version).api;

  if (methodName === 'listInterfaces') {
    return [null, fn(interfaces)];
  } else if (methodName === 'selectApplication') {
    return fn(interfaces, args);
  }
};
...
const onCall = (
  ...
  if (call.interfaceName === 'inspection') {
    [callback.error, callback.result] = inspection(call, applications);
    sendCallback();
    return;
  }

  const app = applications.get(call.application);
...
Server.prototype.listen = function(
  ...
  // If `port` is a number or string which represents a number
  // that means that `port` variable is a TCP port,
  // else it is an IPC path
  const isTcpPort = !!Number(port);
  const listenOptions = [port];

  if (typeof host !== 'function') {
    listenOptions.push(host);
  } else {
    callback = host;
    host = 'localhost';
  }
  ...
  this.transport.listen(...listenOptions, () => {
    this.active = true;
...
Server.prototype.buildApplicationsIndex = apps => {
  ...
  apps.forEach(app => {
    ...
    const versions = new Map([[version || DEFAULT_VERSION, app]]);
...
//
Server.prototype.addApplication = function(application) {
  ...
  const versions = new Map([[version || DEFAULT_VERSION, application]]);
  versions.set('latest', application);
  this.applications.set(name, versions);
};
