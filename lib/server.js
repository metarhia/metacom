...
class Server {
  constructor(config, application) {
    ...
    const { host, balancer, protocol, ports, queue, cors = false } = config;
    HEADERS['Access-Control-Allow-Origin'] = !cors ? '*' : cors.origin;
  ...
  listener(req, res) {
    const { url } = req;
    const channel = transport.http.createChannel(
      this.application,
      req,
      res,
      HEADERS
