...
const { Emitter, jsonParse, parseParams, parseHost, split } = metautil;
const { isError, receiveBody, sample, delay, generateUUID } = metautil;
const ws = require('ws');
...
class Server extends Emitter {
  ...
  constructor(context, options) {
    ...
    const { retry, kind, cors } = options;
    if (retry) this.#retry = retry;
    if (kind === 'balancer') this.#balancer = true;
    this.#console = context.console;
    this.#headers = buildHeaders(cors);
    this.#init();
  }
  ...
  async #handleHttpRequest(req, res) {
    ...
    if (!api && !(this.#balancer && req.url === '/')) {
      if (context.static.constructor.name !== 'Static') return;
      return void context.static.serve(req.url, transport);
    }
    if (this.#balancer) this.#balancing(transport);
  ...
  #handleEventConnection(port) {
    const options = { console: this.#console };
    const transport = new ServerEventTransport(port, options);
    const client = this.#addClient(transport);
  ...
  listen() {
    const { host, port, timeouts } = this.#options;
  ...
  #balancing(transport) {
    const host = parseHost(transport.req.headers.host);
