...
class Client extends Emitter {
  ...
  createStream(name, size) {
    ...
    const id = this.#transport.server.generateId();
...
class Server {
  ...
  init() {
    ...
    this.httpServer.on('request', async (req, res) => {
      ...
      if (!api && !(balancer && url === '/')) this.static(url, transport);
      ...
      if (url === '/api') {
        if (method !== 'POST') transport.error(403);
        else this.message(client, data);
        return;
      }

      this.webhook(client, transport, data);
    });
  ...
  static(url, transport) {
    if (this.application.static.constructor.name !== 'Static') return;
    return void this.application.static.serve(url, transport);
  }
  ...
  message(client, data) {
    ...
    if (type === 'call' && id && method) {
      const procedure = this.procedure(method);
      if (!procedure) return void client.error(404, { id });
      return void this.invoke(client, procedure, packet);
    } else if (type === 'stream' && id) {
      return void this.stream(client, packet);
    }
  ...
  procedure(method) {
    ...
    const procedure = this.application.getMethod(unit, ver, methodName);
    if (!procedure) return null;
    return procedure;
  }

  async invoke(client, procedure, packet) {
    const { id, method, args } = packet;
    const context = client.createContext();

    if (!client.session && procedure.access !== 'public') {
      return void client.error(403, { id });
    }

    try {
      await procedure.enter();
    } catch {
    ...
    let result = null;
    try {
      result = await procedure.invoke(context, args);
    } catch (error) {
  ...
  webhook(client, transport, data) {
    const id = this.generateId();
    const { url } = transport.req;
    const pathname = url.slice('/api/'.length);
    const [method, params] = metautil.split(pathname, '?');
    const procedure = this.procedure(method);
    if (!procedure) return void client.error(404, { id });
    const { protocols } = procedure;
    const webhook = protocols.includes('http') || protocols.includes('https');
    if (!webhook) return void client.error(405, { id });
    const { headers, method: verb } = transport.req;
    const body = metautil.jsonParse(data) || {};
    const parameters = metautil.parseParams(params);
    const redirect = (location) => transport.redirect(location);
    const req = { method: verb, headers, parameters, body };
    const res = { redirect };
    const packet = { id, method, args: { req, res } };
    this.invoke(client, procedure, packet);
  }
