...
class Server {
  ...
  request(client, transport, data) {
    ...
    const procedure = application.getMethod(unit, '*', method);
    if (!procedure) return void client.error(404);
    ...
    const res = { redirect };
    const packet = { path, req, res };
    const { protocols } = procedure;
    const webhook = protocols.includes('http') || protocols.includes('https');
    if (webhook) return void this.webhook(client, procedure, packet);
    const error = new Error('Webhook is not allowed');
    client.error(405, { error });
  }

  async webhook(client, proc, packet) {
    const { path } = packet;

    if (proc.access !== 'public') {
      return void client.error(403, { path });
    }

    try {
      await proc.enter();
    } catch {
      return void client.error(503, { path });
    }
    ...
    try {
      ...
      const context = client.createContext();
      result = await proc.invoke(context, { req, res });
    } catch (error) {
      client.error(500, { path, error });
    } finally {
      proc.leave();
    }
    ...
    if (metautil.isError(result)) {
      const { code, httpCode = 200 } = result;
      return void client.error(code, { path, error: result, httpCode });
    }

    client.send(result);
    this.console.log(`${client.ip}\t${path}`);
  }
