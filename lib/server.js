...
class Server {
  ...
  request(channel) {
    ...
    const { metacomUrl, routerUrl } = this.options;
    if (req.url === metacomUrl && req.method !== 'POST') {
      channel.error(403);
      return;
    }
    const body = metautil.receiveBody(req);
    if (req.url === metacomUrl) {
      body.then((data) => {
      ...
      body.then((data) => {
        ...
        const routerPath = req.url.slice(routerUrl);
        if (routerPath) channel.route(routerPath, req);

        const pathname = req.url.slice(metacomUrl.length);
