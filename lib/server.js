...
class Server {
  ...
  async rpc(client, packet) {
    const [unitName, methodName] = packet.method.split('/');
    const [unit, ver = '*'] = unitName.split('.');
    const proc = this.application.getMethod(unit, ver, methodName);
    return void this.handleRequest(client, packet, proc);
  }
  ...
  request(client, transport, data) {
    ...
    if (hook) this.handleRequest(client, { ...packet, verb, headers }, hook);
  ...
  async handleRequest(client, packet, proc) {
    ...
    try {
      ...
      if (type !== 'call') return void client.error(503, { id, error });
