...
metatests.testSync('parser.readStructType', test => {
  const handshake = Buffer.alloc(10);
  handshake.writeIntLE(STRUCT_HANDSHAKE, 0, 1);
  ...
  const pong = Buffer.alloc(10);
  pong.writeIntLE(STRUCT_PONG, 0, 1);
...
metatests.testSync('parser.parseHandshake', test => {
  ...
  buffer.writeIntLE(STRUCT_HANDSHAKE, 0, 1);
  buffer.writeIntLE(status, 3, 1);
  token.copy(buffer, 4);

  const handshake = parser.parseHandshake(buffer);

  test.strictSame(handshake.structType, STRUCT_HANDSHAKE);
...
metatests.testSync('parser.parseParcelHeader', test => {
  ...
  buffer.writeIntLE(STRUCT_PARCEL_HEADER, 0, 1);
  ...
  test.strictSame(parcel.structType, STRUCT_PARCEL_HEADER);
